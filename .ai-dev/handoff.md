...
| **Phase 1D** | [plan.md](./tasks/phase-1d-organization/plan.md) | 組織 scoping + 持久化 |

Phase 1 全部完成後進入 Phase 2 模組硬化。

### 派工指令範本

```text
請依 .ai-dev/tasks/phase-1d-organization/plan.md 執行。
先讀 docs/02-project-structure.md（org scoping 章節）。
...
| 組織管理 | 🔴 | Phase 1D 進行中：scoping、持久化、假報表 |
...
1. **`npm run types` 仍失敗（128 errors）** — Organization/Users/Departments 留 1D/Phase 2。
2. ~~**Dashboard 假操作**~~ — Phase 1C 已驗收。
3. ~~**表單前端路由缺口**~~ — Phase 1B 已驗收。
4. **組織設定 demo 化** — Phase 1D 進行中。
...
*Phase 1C 已驗收。下一階段：派工 Phase 1D → `.ai-dev/tasks/phase-1d-organization/plan.md`*
