...
| 🟡 P1 | **3D** Notifications 驗證 | [plan](./tasks/phase-3d-notifications-verification/plan.md) |
| 🟢 P2 | **3E** Reports / 分頁 | [plan](./tasks/phase-3e-reports-pagination/plan.md) |
...
| 用戶 / 部門 | 🟡 | **3B** Layout 統一；**3C** DepartmentPagesTest（244 tests） |
...
| 3 | **3D + 3E** | Notifications + Reports polish（可並行） |
| 4 | **3F** | 響應式審查 |
| 5 | **3G** | 部署文件（可隨時） |
...
```bash
php artisan test                    # 244 passed / 1354 assertions
npm run build                       # 通過
...
*Phase 3A–3C 已驗收（244 tests）。建議並行派 3D + 3E。*
