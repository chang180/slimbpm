...
## 最後更新：2026-06-06（Phase 2E 驗收通過 — Phase 2 完成）
...
Phase 3 項目見 [`docs/07-roadmap.md`](../docs/07-roadmap.md) — 回歸測試、響應式審查、部署指南。
...
| 工作流程 | 🟡 | Phase 2D：Monitor 操作 UX 已修 |
| 報表 `/reports/*` | 🟡 | Phase 2D：Index 匯出已接線；子頁原已正確 |
...
2. **Layout 不統一** — Users/Departments Index/Show 仍用 `AuthenticatedLayout`（Phase 3 polish）。
3. **`@/lib/route.ts` 不完整** — 部分 Organization 頁面 submit 可能仍 broken。
...
| 順序 | 任務 | 說明 |
|------|------|------|
| 1 | 回歸測試 | 已修模組補 Feature/Inertia 測試 |
| 2 | 響應式審查 | Dashboard、Forms、Workflow 頁面 |
| 3 | 部署指南 | env、queue、mail、DB 遷移 |
| 4 | Layout polish | Users/Departments Index/Show → AppLayout |
...
```bash
php artisan test                    # 236 passed / 1253 assertions
npm run build                       # 通過
npm run types                       # 0 errors（Phase 2E）
...
*Phase 2 已全部驗收（TS 176→0）。下一階段：Phase 3 → `docs/07-roadmap.md`*
