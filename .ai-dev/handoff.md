...
| ~~Phase 2A~~ | [plan.md](./tasks/phase-2a-users/plan.md) | ✅ 已驗收（Users TS 30→0；全專案 38→9） |
| **Phase 2B** | [plan.md](./tasks/phase-2b-departments/plan.md) | Departments JSX 型別修復 |
...
```text
請依 .ai-dev/tasks/phase-2b-departments/plan.md 執行。
先讀 docs/03-module-status.md（Departments 章節）。
...
| 用戶 / 部門 | 🟡 | Users Phase 2A 完成（Create/Edit 可 render）；Departments JSX 型別待修（2B） |
| 邀請 / 通知 | 🟡 | 獨立頁面有；Dashboard 已接邀請 API |
...
1. **`npm run types` 仍失敗（9 errors）** — Departments JSX（2B）、invitations 型別（2C）、app-header 等（2E）。
2. ~~**Users Create/Edit 白屏**~~ — Phase 2A 已驗收。
3. ~~**Dashboard 假操作**~~ — Phase 1C 已驗收。
4. ~~**表單前端路由缺口**~~ — Phase 1B 已驗收。
5. ~~**組織設定 demo 化**~~ — Phase 1D 已驗收。
...
*Phase 2A 已驗收。下一階段：派工 Phase 2B → `.ai-dev/tasks/phase-2b-departments/plan.md`*
