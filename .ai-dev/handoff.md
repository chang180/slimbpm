...
| ~~Phase 1B~~ | [plan](./tasks/phase-1b-forms/plan.md) | ✅ 已驗收（164→131 TS；Forms 0 error） |
| **Phase 1C** | [plan.md](./tasks/phase-1c-dashboard/plan.md) | 儀表板真操作 + 壞連結 |

Phase 1D 等 1C 驗收後由協調者撰寫 plan。

### 派工指令範本

```text
請依 .ai-dev/tasks/phase-1c-dashboard/plan.md 執行。
先讀 docs/03-module-status.md（Dashboard 章節）。
...
| 儀表板 | 🔴 | Phase 1C 進行中：假 handler、壞連結 |
...
1. **`npm run types` 仍失敗（131 errors）** — Forms 已清；Dashboard/Org 留 1C–1D。
2. **Dashboard 假操作** — Phase 1C 進行中。
...
*Phase 1B 已驗收。下一階段：派工 Phase 1C → `.ai-dev/tasks/phase-1c-dashboard/plan.md`*
