...
1. **`npm run types` 失敗** — Vite build 可過，不代表型別安全。
2. **Dashboard 假操作** — `useDashboardActions.ts`、`QuickActions.tsx`、`WorkflowMenu.tsx`。
3. **表單路由缺口** — 缺 Edit 頁、`route.ts` 不完整、Wayfinder 路由名稱衝突。
...
*下次接手：先讀本文件 + `docs/07-roadmap.md`，從 Phase 1A 開始。*
